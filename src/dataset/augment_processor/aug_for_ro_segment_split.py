...
class AugForROSegmentSplit(object):
    '''
    segment进行随机切分
    注意：这里需要同时兼容4点和2点坐标
    '''
    ...
    def process(self, sample):
        ...
        self.re_gen_labels(json_obj)

        return sample

    def re_gen_labels(self, json_obj):

        if 'label_segment_order' not in json_obj or len(json_obj['label_segment_order']) == 0:
            return
        label_segment_order = json_obj['label_segment_order']

        id2order_id = dict()
        for order_id, id in enumerate(label_segment_order):
            id2order_id[id] = order_id

        order_list = []
        for seg in json_obj['document']:
            id = seg['id']
            ori_id = id
            if 'ori_id' in seg:
                ori_id = seg['ori_id']
            order_id = id2order_id[ori_id]

            order_list.append([order_id, id])

        # 排序
        order_label = sorted(order_list, key=operator.itemgetter(0, 1))

        label_segment_order = [v[1] for v in order_label]

        json_obj['label_segment_order'] = label_segment_order

    def box_split(self, box, ratio):
        '''
        这里只支持4点坐标
        '''
        assert isinstance(box[0], list), '只支持4点式坐标'
    ...
    def rnd_split(self, obj):
        ...
        for seg_idx in range(len(document)):
            ...
            if len(words) > 1 and rnd.random() < self.prob:
                ...
                '''
                这里需要兼容没有 word box的情况
                '''
                has_word_box = True if 'box' in words[0] and len(words[0]['box']) > 0 else False

                if has_word_box:

                    # 使用word的box计算seg box
                    box_a = [
                        words_a[0]['box'][0], words_a[-1]['box'][1],
                        words_a[-1]['box'][2], words_a[0]['box'][3]
                    ]

                    box_b = [
                        words_b[0]['box'][0], words_b[-1]['box'][1],
                        words_b[-1]['box'][2], words_b[0]['box'][3]
                    ]
                else:
...
if __name__ == '__main__':
    ...
    obj = json.loads(t)
    # show_cdip(obj, '0.jpg')
    aug = AugForROSegmentSplit(prob=0.5)
    aug.process({'json': obj})
