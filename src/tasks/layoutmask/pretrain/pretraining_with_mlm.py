...
from src.model.layoutmask.modeling_layoutmask import LayoutMaskModelForPretrainingMLM
...
class LayoutMaskPretrainModule(pl.LightningModule):
    def __init__(self,
        ...
        self.model = LayoutMaskModelForPretrainingMLM.from_pretrained(config=self.config,
    ...
    def training_step(self, batch, batch_idx):
        ...
        #print(f'Start training for the {self.global_step} step')

        outputs = self(**batch)
        mlm_loss = outputs['mlm_loss']

        loss = mlm_loss * self.hparams.mlm_task_weight
