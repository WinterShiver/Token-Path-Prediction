...
from src.dataset.feature_processor.layoutlm_v3.ner_tpp_gb_processor import LayoutLMv3ForTPPGPNerProcessor
from src.model.layoutlm_v3.configuration_layoutlmv3 import LayoutLMv3Config
from src.model.layoutlm_v3.modeling_layoutlmv3 import LayoutLMv3ForTokenPathPrediction
...
class CodeDocNERGBModelModule(pl.LightningModule):
    def __init__(self,
        ...
        self.model = LayoutLMv3ForTokenPathPrediction.from_pretrained(config=self.config,
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for uid, predict_result in zip(uids, predict_result_batchs):
            ...
            for label, entity, score in predict_result:
                ...
                predict_map[label].append(entity)

            # predict_map的keys排序 每个key下的实体列表也排序
            predict_map_new = dict()
            for k in sorted(predict_map):
                predict_map_new[k] = sorted(predict_map[k])
            final_predict_result.append([uid, predict_map_new])
...
class LightningRunner:
    ...
    def run(self):
        ...
        data_processor_train = LayoutLMv3ForTPPGPNerProcessor(tokenizer=tokenizer,
        ...
        data_processor = LayoutLMv3ForTPPGPNerProcessor(tokenizer=tokenizer,
                                                        ner_labels=ner_labels,
                                                        box_level=self.args.box_level,
                                                        max_text_length=self.args.max_length,
                                                        norm_bbox_height=self.args.norm_bbox_height,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enable_position_1d', type=lambda x: bool(strtobool(x)), 
                        nargs='?', const=True, help='是否使用v3的global 1d编码', default=True)
